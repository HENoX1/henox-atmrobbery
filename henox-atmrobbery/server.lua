local QBCore = exports['qb-core']:GetCoreObject()
local Cooldown = false
local amount = math.random(15000, 20000)


-- hack
QBCore.Functions.CreateUseableItem('trojan_usb', function(source)
	local Player = QBCore.Functions.GetPlayer(source)
   TriggerClientEvent('hack:trojan_usb',source)
end)

-- Recompensa
RegisterServerEvent("henox-atmrobbery:success")
AddEventHandler("henox-atmrobbery:success",function()
    local src = source
	local Player =  QBCore.Functions.GetPlayer(source)
    local amount = math.random(15000, 20000)
    Wait(45500)

	Player.Functions.AddItem('markedbills', amount)
	--Player.Functions.AddItem("markedbills", tonumber(reward))
    Player.Functions.RemoveItem("trojan_usb", 1)
	--TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items["markedbills"], "add")
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], "add")
 local luck = math.random(1, 100)
 local odd = math.random(1, 100)
 if luck <= 20 then
    Player.Functions.AddItem("security_card_01", 1)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["security_card_01"], "add")
 end
end)

-- Cooldown
RegisterServerEvent('henox-atmrobbery:Server:BeginCooldown')
AddEventHandler('henox-atmrobbery:Server:BeginCooldown', function()
    Cooldown = true
    local timer = 60 * 1000
    while timer > 0 do
        Wait(1000)
        timer = timer - 1000
        if timer == 0 then
            Cooldown = false
        end
    end
end)

QBCore.Functions.CreateCallback("henox-atmrobbery:Cooldown",function(source, cb)
    if Cooldown then
        cb(true)
    else
        cb(false)
        
    end
end)
